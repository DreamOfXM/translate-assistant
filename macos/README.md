...
## 一个可能踩到的坑：重新构建后授权可能失效

App 目前用 **ad-hoc 签名**（`codesign --sign -`）。macOS 的 TCC（也就是「辅助功能」
授权的登记处）识别应用的方式与签名有关，而 ad-hoc 签名每次构建都可能变化，
所以**重新构建后授权有可能失效**。

失效的表现：菜单里显示「未授权」，热键按下去没反应。这时去
**隐私与安全性 → 辅助功能**，把旧条目按 `−` 移除，再按 `+` 把 App 加回来 ——
只切换勾选框不一定够用。

> 实测（macOS 26.6.2）：同一个路径、同一个 bundle id 反复重建时，授权有时能保留下来。
> 所以重建完之后先看一眼菜单里的授权状态，不必每次都重新添加。
