...
- **只翻正文主体**：先用语义容器（`article` / `main` / `[role=main]` / 常见 CMS 类名）找正文根，
  ...
  认不出正文根的页面（比如裸段落直挂 `<body>` 的简单页）、或正文散落在多个体量相当的容器里
  （打分最高的单一容器盖不住页面段落分的大头）都退回「翻译所有合格块」——宁可多翻，不漏翻
- **工具型页面不自动翻**：满页短标签、时间戳、表格数据而没有成段文字的页面
  （GitHub、管理后台），自动模式直接跳过，按钮显示「工具页不自动翻译 · 点击翻正文」，
  手动点仍会执行；屏幕阅读器专用文本（`sr-only`）和表格里的短数据（日期、文件名）一律不翻
